...
const asignarConsultorio = async (id,consultorioId) => {
    try {
      console.log(id)
      const response = await fetch(`https://backend-hospital-8aqk.onrender.com/api/v1/emergencias/consultorioE/${id}?consultorio_id=${consultorioId}`, {
          method: 'PATCH',
          headers: {
      ...
      alert(`Se ha asignado el Consultorio ${consultorioId} con la emergencia  ${id}`)
...
export default function ConsultorioSelectEme() {
  const [emergenciaId,setEmergenciaId] = React.useState(0);
  ...
  return (
    ...
    >
      ...
      <DialogActions>
        ...
        <Button type="submit" onClick={() => asignarConsultorio(emergenciaId,consultorio)}>Asignar</Button>
      </DialogActions>
    </Dialog>
  </React.Fragment> 
  );
